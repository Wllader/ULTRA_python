...
class Widget(pg.sprite.Sprite):
    ...
    def align_center(a:tuple, b:tuple):
        return np.abs(np.array(a) - np.array(b)) // 2
...
class Button(Widget):
    def __init__(self, screen, pos, size, text:str, font=None, fg:np.ndarray=BLACK, bg:np.ndarray=WHITE, command:Callable=lambda: print("Clicked!")):
    ...
    @property
    def current_bg(self):
        return self.bg * (1 if not self.mouse_held[0] else 0.8)

    @property
    def image(self):
        self._image.fill(self.current_bg)
...
class Entry(Widget):
    def __init__(self, screen, pos, size, font=None, fg:np.ndarray=WHITE, bg:np.ndarray=GREY * 28, default_text="", tooltip_text=""):
        ...
        self.font = font if font else pg.font.SysFont("Calibri", self.size[1] // 2)
        self.fg = fg
        self.bg = bg
