...
class PongBot(PongEntity):
    def __init__(self, screen, size, init_center_position, speed, color):
        super().__init__(screen, size, init_center_position, speed, color)

        self.target:PongEntity = None

    def update(self, dt): #todo Drift towards a ball / center if ball is flying away from us
        if self.target:
