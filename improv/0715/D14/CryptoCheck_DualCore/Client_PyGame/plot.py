...
class Particle(Widget):
    def __init__(self, screen, pos, size, color=BLUE):
        ...
        self.color_tween = Tween.EaseInOutCubic(1500)
        self.pos_tween = Tween.EaseInOutCubic(1500)
...
class Plot(Widget):
    def __init__(self, screen, pos, size):
        ...
        self.particles = pg.sprite.Group(*[
            Particle(self._image, np.array([w*i, 0]), np.array([w, h])) for i in range(0, size[0], w)
        ])
