...
class Fetcher(ABC):
    def __init__(self, cache_db_path:str, expiration_s:int=180):
        super().__init__()
    ...
    def get_json(self, tick, params) -> dict:
        df:pd.DataFrame = self.get(tick, params)
...
class CryptoFetcher(Fetcher):
    def fetch_data(self, params, endpoint) -> dict[str, Any]:
    ...
    def get(self, tick, params) -> pd.DataFrame|None:
        ...
        with sqlite3.connect(self.cache_db) as conn:
            ...
            data = self.fetch_data(params, endpoint)
