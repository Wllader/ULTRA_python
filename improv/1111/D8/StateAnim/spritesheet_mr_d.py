...
class SpriteSheet:
    def __init__(self, path:str, size:tuple[int], scale:float=1., default_index:np.ndarray = np.zeros(2, dtype=int)):
    ...
    def get_image(self, index:np.ndarray=None) -> pg.Surface:
    ...
    def frame(self):
        ...
        if self.current_frame_time >= self.frame_time:
            ...
            self.frame_index = (self.frame_index + index_diff) % self.current_animation_lenght
